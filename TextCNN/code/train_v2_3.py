    ...
    with tf.Session(config=config) as sess:
        ...
        for x_data, y_data, batch_nums in batch_iterator(sub_sample, sub_label):
            ...
            _, train_loss, train_acc, current_step = sess.run(
                [text_cnn.optimizer, text_cnn.loss, text_cnn.accuracy, text_cnn.global_step], feed_dict=feed_dict)
            ...
            if len(loss_list) == batch_nums:
                ...
                print("%s the step is %s , the mean train loss is %s , the  mean train acc is %s" % (
                time.strftime("%Y-%m-%d %H:%M:%S", time.localtime()), current_step, loss_mean, acc_mean))
                loss_list = []
                acc_list = []
                ...
                if break_step == 10:
                    ...
                    tf.train.Saver().save(sess, save_path="../model/mod_v2/NLP" + str(model_number),
...
def main():
    text_cnn_3 = TextCNN(max_document_length=max_document_length,
    ...
    print("训练 text_cnn_3")
    train(text_cnn_3, embedding_matrix, 3)
